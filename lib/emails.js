...
export function buildCommitmentNudgeEmail({ taskTitle, commitmentTime, taskId, userEmail }) {
  const planUrl = `${BASE_URL}/planner?task=${taskId}`;
  ...
  const content = `
    ...
    <tr>
      <td style="padding:32px;">
        ...
        <!-- Headline -->
        <h1 style="margin:0 0 8px 0;font-size:24px;font-weight:800;color:#ffffff;line-height:1.3;">
          You said you'd start by ${commitmentTime}.
        </h1>
        <p style="margin:0 0 24px 0;font-size:16px;color:#94a3b8;line-height:1.5;">
          Your task <strong style="color:#e2e8f0;">"${taskTitle}"</strong> is ready and waiting.
  ...
  return {
    subject: `You committed to starting "${taskTitle}" — let's go`,
    html: emailWrapper(content),
  };
}
...
export function buildFriendNudgeEmail({ senderUsername, receiverName, message }) {
  ...
  const messageBlock = message
    ? `<p style="margin:0 0 24px 0;font-size:16px;color:#e2e8f0;line-height:1.5;background-color:#0f172a;border-left:3px solid #34d399;border-radius:0 8px 8px 0;padding:12px 16px;font-style:italic;">"${message}"</p>`
    : '';
  ...
  const content = `
    ...
    <tr>
      <td style="padding:32px;">
        ...
        <!-- Headline -->
        <h1 style="margin:0 0 8px 0;font-size:24px;font-weight:800;color:#ffffff;line-height:1.3;">
          @${senderUsername} says: get moving!
        </h1>
        <p style="margin:0 0 24px 0;font-size:16px;color:#94a3b8;line-height:1.5;">
          Hey ${receiverName}, your friend is checking in. They believe in you — now prove them right.
        </p>
        ...
        <p style="margin:20px 0 0 0;font-size:13px;color:#475569;">
          One step is all it takes. Show @${senderUsername} what you're made of.
        </p>
  ...
  return {
    subject: `@${senderUsername} says: get moving!`,
    html: emailWrapper(content),
  };
}
...
export function buildFriendPraiseEmail({ senderUsername, receiverName, message }) {
  ...
  const messageBlock = message
    ? `<p style="margin:0 0 24px 0;font-size:16px;color:#e2e8f0;line-height:1.5;background-color:#0f172a;border-left:3px solid #fbbf24;border-radius:0 8px 8px 0;padding:12px 16px;font-style:italic;">"${message}"</p>`
    : '';
  ...
  const content = `
    ...
    <tr>
      <td style="padding:32px;">
        ...
        <!-- Headline -->
        <h1 style="margin:0 0 8px 0;font-size:24px;font-weight:800;color:#ffffff;line-height:1.3;">
          @${senderUsername} says: nice work!
        </h1>
        <p style="margin:0 0 24px 0;font-size:16px;color:#94a3b8;line-height:1.5;">
          Hey ${receiverName}, someone noticed your progress. Keep that momentum going.
        </p>
  ...
  return {
    subject: `@${senderUsername} says: nice work!`,
    html: emailWrapper(content),
  };
}
